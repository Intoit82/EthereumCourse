    ...
    describe("Vehicle Operations", function() {
        ...
        describe("enterRoad", function() {
            ...
            it("should be possible to enter road with more than required deposit", function() {
                ...
                console.log("deposit: " +(deposit0 * multiplier0) + 1)
            ...
            it("should be possible to report exit road on route with known price below deposited", function() {
                return operator.setRoutePrice(booth0, booth1, deposit0, { from: owner1 })
                    ...
                    .then(tx => {
                        ...
                        assert.strictEqual(logExited.args.finalFee.toNumber(), deposit0 * multiplier0,"Failed 1");
                        assert.strictEqual(logExited.args.refundWeis.toNumber(), extraDeposit * multiplier0,"Failed 2");
                        // console.log(tx.receipt.gasUsed);
                    ...
                    .then(info => {
                        assert.strictEqual(info.hashed0[0], vehicle0,"Failed 10");
                        assert.strictEqual(info.hashed0[1], booth0);
                        assert.strictEqual(info.hashed0[2].toNumber(), 0,"Failed 11");
                        assert.strictEqual(info.pendingCount01.toNumber(), 0,"Failed 12");
                        assert.strictEqual(info.pendingCount02.toNumber(), 0,"Failed 13");
                        return Promise.allNamed({
                    ...
                    .then(balances => {
                        assert.strictEqual(balances.operator.toNumber(), deposit0 * multiplier0,"Failed 5");
                        assert.strictEqual(balances.collected.toNumber(), deposit0 * multiplier0,"Failed 6");
                        assert.strictEqual(
                            balances.vehicle0.toString(10),
                            vehicleInitBal.plus(extraDeposit * multiplier0).toString(10),"Failed 7");
                    });
            });
            ...
            describe("Clear one more pending payment", function() {
                ...
                it("should be possible to set the base route price below both deposits then clear the second by hand", function() {
                    return operator.setRoutePrice(booth0, booth2, deposit0, { from: owner1 })
                        ...
                        .then(tx => {
                            assert.strictEqual(tx.receipt.logs.length, 1,"Length receipt");
                            assert.strictEqual(tx.logs.length, 1,"length logs");
                            const logExited = tx.logs[0];
                            assert.strictEqual(logExited.event, "LogRoadExited");
                            assert.strictEqual(logExited.args.exitBooth, booth2, "Failed 1");
                            assert.strictEqual(logExited.args.exitSecretHashed, hashed0, "Failed 2");
                            assert.strictEqual(logExited.args.finalFee.toNumber(), deposit0 * multiplier0, "Failed 3");
                            assert.strictEqual(
                                logExited.args.refundWeis.toNumber(),
                                (extraDeposit0 - deposit0) * multiplier0, "Failed 4");
                            // console.log(tx.receipt.gasUsed);
                            return Promise.allNamed({
                                hashed0: () => operator.getVehicleEntry(hashed0),
                                hashed1: () => operator.getVehicleEntry(hashed1),
                        ...
                        .then(info => {
                            assert.strictEqual(info.hashed0[0], vehicle0, "Failed 5");
                            assert.strictEqual(info.hashed0[1], booth0, "Failed 6");
                            assert.strictEqual(info.hashed0[2].toNumber(), 0, "Failed 7");
                            assert.strictEqual(info.hashed1[0], vehicle1, "Failed 8");
                            assert.strictEqual(info.hashed1[1], booth0, "Failed 9");
                            assert.strictEqual(info.hashed1[2].toNumber(), 0, "Failed 10");
                            assert.strictEqual(info.pendingCount01.toNumber(), 0, "Failed 11");
                            assert.strictEqual(info.pendingCount02.toNumber(), 0, "Failed 12");
                            return Promise.allNamed({
                                operator: () => web3.eth.getBalancePromise(operator.address),
                                collected: () => operator.getCollectedFeesAmount(),
                                vehicle0: () => web3.eth.getBalancePromise(vehicle0),
                        ...
                        .then(balances => {
                            assert.strictEqual(
                                balances.operator.toNumber(),
                                deposit0 * (multiplier0 + multiplier1), "Failed 13");
                            assert.strictEqual(balances.collected.toNumber(), deposit0 * (multiplier0 + multiplier1), "Failed 14");
                            assert.strictEqual(
                                balances.vehicle0.toString(10),
                                vehicle0InitBal.plus((extraDeposit0 - deposit0) * multiplier0).toString(10), "Failed 15");
                            assert.strictEqual(
                                balances.vehicle1.toString(10),
                                vehicle1InitBal.plus((extraDeposit1 - deposit0) * multiplier1).toString(10), "Failed 16");
                        });
                });
